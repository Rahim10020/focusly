    ...
    >
      ...
      <div className="flex items-start gap-3 mb-3">
        <button
          ...
          className={`shrink-0 w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all cursor-pointer ${isDone
      ...
      <div className="flex items-center gap-1 mt-3 pt-3 border-t border-border opacity-0 group-hover:opacity-100 transition-opacity">
        <Button variant="ghost" size="sm" onClick={() => onEdit(task)} className="text-xs">
          Edit
        </Button>
        {!isActive && !isDone && (
          <Button variant="ghost" size="sm" onClick={() => onSelect(task.id)} className="text-xs">
            Set Active
          </Button>
        )}
        {isActive && (
          <Button variant="ghost" size="sm" onClick={onUnselect} className="text-xs">
            Unset
          </Button>
        )}
        <Button variant="ghost" size="sm" onClick={() => onDelete(task.id)} className="text-xs text-error hover:bg-error/10">
          Delete
        </Button>
      </div>
    </div>
