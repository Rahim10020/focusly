...
export function useTaskNotifications({
  ...
  useEffect(() => {
    ...
    const checkUpcomingTasks = () => {
      const now = Date.now();
      const oneHourFromNow = now + TIME_MS.HOUR;
      ...
      tasks.forEach((task) => {
        ...
        if (task.startTime) {
          ...
          else if (timeUntilStart >= -TIME_MS.MINUTE && timeUntilStart <= 0) {
            showNotification(
      ...
      tasks.forEach((task) => {
        ...
        if (task.dueDate < now) {
          const daysOverdue = Math.floor((now - task.dueDate) / TIME_MS.DAY);
          if (daysOverdue === 0) {
            showNotification(
    ...
    const showNotification = (title: string, body: string, taskId: string) => {
      if ("Notification" in window && Notification.permission === "granted") {
        ...
        if (lastShown && Date.now() - parseInt(lastShown) < TIME_MS.HOUR) {
    ...
    const interval = setInterval(checkUpcomingTasks, 5 * TIME_MS.MINUTE);
