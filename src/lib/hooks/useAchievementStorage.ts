...
export function useAchievementStorage(): UseAchievementStorageReturn {
  ...
  const [localAchievements, setLocalAchievements] = useLocalStorage<
    Achievement[]
  >(
  ...
  const setAchievements: Dispatch<SetStateAction<Achievement[]>> =
    isAuthenticated ? setDbAchievements : setLocalAchievements;
