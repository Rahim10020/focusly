...
const STORAGE_KEY = STORAGE_KEYS.POMODORO_STATE;
...
export function usePomodoro(options: UsePomodoroOptions) {
  ...
  useEffect(() => {
    if (
      ...
      persistedState.currentSessionStart &&
      persistedState.timeLeft !== undefined
    ) {
      const elapsed = Math.floor(
        (Date.now() - persistedState.currentSessionStart) / 1000,
      );
  ...
  const handleSessionComplete = useCallback(() => {
    ...
    if (sessionType === "work") {
      ...
      const isLongBreak =
        newCompletedCycles % settings.cyclesBeforeLongBreak === 0;
  ...
  useEffect(() => {
    if (
      ...
      const timeoutId = setTimeout(() => setStatus("running"), TIME_MS.SECOND);
